        ...
        if (dialog.length > 0) {
            ...
            if (Notification.permission === 'granted') {
                var n = new Notification('SHOWROOM', {
                    ...
                    icon: chrome.extension.getURL('img/icon80x80.png'),
                    tag: 'showroom-bonus-notification'
                });
                // 通知クリック時にタブをアクティブ化する
                // NOTE: chrome.tabs APIは content script からはアクセスできないため、
                //       一度バックグラウンドにメッセージを飛ばし、background script から
                //       タブのアクティブ化を行う。
                n.addEventListener('click', function(ev) {
                    chrome.runtime.sendMessage('activate tab');
                    // 通知を閉じる
                    n.close();
                });
            }
